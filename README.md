...
2. **入座发牌**：准备阶段可点自己站起围观、点空位坐下或换座；满座时加入会先围观。座位坐满且入座玩家全员准备后由房主发放身份；每人在自己的手机上查看身份和初始视野。
3. **按需操作**：线下讨论，房主根据现场进度发起投票、任务或对应板子的技能操作。
4. **结束再开**：线下完成最终结算，由房主结束本局；可在同一牌桌重新准备、再开一局。

「我的牌桌」显示房主、公共状态、人数、我的座位和最近活动，并支持仅自己可见的30字备注。进行中的牌桌优先，同状态按最近进入排序；达到6桌时显示状态筛选。切换牌桌不会退出当前对局。

每桌的更多菜单区分三种操作：**移除记录**仅隐藏自己的列表条目（8秒内可撤销，重新进入后恢复），**离开房间**按原游戏规则退出并释放座位，**解散房间**仅房主可执行且需二次确认，会影响所有成员并删除对局记录。移除记录和个人备注保存在服务端，刷新和重启不会丢失。失效房间保留可清理的条目；没有牌桌时可直接选择加入或创建。房主仍可转交管理权，并在准备阶段或对局结束后从房间设置移出玩家。
...
| 十二骑士 | 12              | 秘密技能提交与同时生效、守护／换号／替死、猎人链、B 牌复活、身份转换、仙女与先知视野 |
...
陪测面板仅操作独立测试账号，不代替真人提交。默认隐藏测试玩家身份，可手动展开；清空陪测玩家时会保留真人和牌桌。陪测担任房主时可直接发起投票、任务、技能、查验、结算及转交房主。管理房间列表提供带房间号的陪测入口；面板按房间保存当前标签页的账号和未确认请求，切换无需先清空旧房间。房间或凭据失效时可移除本地记录（不删除服务器座位）；残留座位由管理平台在准备阶段清理。面板仅允许本机访问，且必须同时启用 `DEV_AUTH=1`、`DEV_PANEL=1`；生产环境不可用。
