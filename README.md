...
「我的牌桌」可找回已加入的房间。切换牌桌不会退出当前对局；准备阶段可离席，房主还可转交管理权或删除牌桌。

## 功能亮点

- **私密身份与视野**：身份默认遮盖，主动查看；小程序切到后台会清除私密展示内容。
- **自由安排流程**：投票可独立发起，任务自选队员及一张／两张失败票门槛，不强制按固定阶段推进。
- **秘密提交、统一结算**：任务仅等待队员提交，结算后展示汇总；房主能查看完成进度，不能查看秘密票型或目标。
...
所有板子均以**按需工具、房主手动结束**为主：不会因为三次任务成功／失败、连续否决或单次操作结果自动结束整局。当前小程序不提供刀梅林入口，最终刀梅林在线下进行。影中执刃的完全自动胜负模式未开放。
