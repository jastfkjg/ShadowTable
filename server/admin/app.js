...
async function refresh() {
  ...
  try {
    const [{ rooms, total }, { entries }] = await Promise.all([
    ...
    for (const entry of entries) {
      const item = el("li", "");
      item.append(
        el("time", new Date(entry.created).toLocaleString()),
        el(
          "div",
          `${labels[entry.action] || entry.action}${entry.code ? " · " + entry.code : ""}${entry.reason ? " · " + entry.reason : ""}`,
        ),
      );
      $("audit").append(item);
    }
